      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="grid gap-10 md:grid-cols-[0.8fr_1.2fr] md:items-start">
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-primer/70">
          ...
          <ol className="grid gap-5 sm:grid-cols-2">
            {MILESTONES.map((item) => (
              <li key={item.year} className="rounded-xl border border-charcoal-line bg-charcoal-soft p-5">
                <p className="font-display text-lg text-signal-gold">{item.year}</p>
                <p className="mt-1.5 text-sm text-primer/70">{item.detail}</p>
              </li>
            ))}
          </ol>
        </div>
